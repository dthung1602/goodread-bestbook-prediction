...
```{r}
ds=read.table("tmp5.csv",sep=",",header=TRUE)
```
...
```{r}
ds <- transform(ds, Ngữ.văn = as.numeric(Ngữ.văn), 
                    Vật.lí = as.numeric(Vật.lí),
                    Hóa.học = as.numeric(Hóa.học),
                    Sinh.học = as.numeric(Sinh.học),
                    KHTN = as.numeric(KHTN),
                    Lịch.sử  = as.numeric(Lịch.sử ),
                    Địa.lí = as.numeric(Địa.lí),
                    GDCD = as.numeric(GDCD),
                    KHXH = as.numeric(KHXH),
                    Tiếng.Anh = as.numeric(Tiếng.Anh))
```
```{r}
cor_tds <- cor(ds, ds, method = "pearson")
cor_df<- data.frame(cor=cor_tds[1:10,11], varn = names(cor_tds[1:10,11])) 
...
```{r}
model <- lm(data = pca_df, Tiếng.Anh ~ .)
summary(model)
```
